...
import { Local} from '../interfaces/Local'; // Importe Local e Avaliacao
import { Avaliacao } from '@/interfaces/Avaliacao';

// Define as props que a Home Page vai receber, provenientes do getServerSideProps.
interface HomePageProps {
  locals: Local[];
  avaliacoes: Avaliacao[];
  localMelhorAvaliado: Local[];
  error?: string; // Para exibir mensagens de erro da API
}

// O componente da página principal.
const HomePage: React.FC<HomePageProps> = ({ locals, avaliacoes, localMelhorAvaliado, error }) => {
  // Se houver um erro na requisição da API, exibe uma mensagem de erro.
  if (error) {
    return (
      <Layout>
        <section className="py-20 text-center text-red-600 bg-red-50 rounded-lg mx-auto max-w-xl p-8 shadow-md mt-20">
          ...
          <p className="mt-6">Por favor, verifique se a URL da sua API está correta e se a API está funcionando.</p>
        </section>
      </Layout>
    );
  }

  // Garante que as props são arrays, mesmo que venham como undefined por algum motivo inesperado.
      ...
      <section id="locais" className="py-20 bg-gray-50">
        <div className="container mx-auto px-6">
          ...
          {safeLocations.length > 0 ? (
            ...
            <p className="text-center text-xl text-gray-600 mt-10">Nenhum local disponível no momento. Volte em breve!</p>
          )}
        </div>
      </section>
      ...
      <section id="avaliacoes" className="py-20 bg-white">
        <div className="container mx-auto px-6">
          ...
          {safeRecentReviews.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {safeRecentReviews.map((avaliacao) => (
                <ReviewCard key={avaliacao.id} avaliacao={avaliacao} />
              ))}
            </div>
          ) : (
            <p className="text-center text-xl text-gray-600 mt-10">Ainda não há avaliações recentes. Seja o primeiro a avaliar!</p>
          )}
        </div>
      </section>

      {/* Seção 3: Ranking de Locais (adaptado sem RatingItem) */}
        ...
        <div className="container mx-auto px-6">
          ...
          {safelocalMaisAvaliado.length > 0 ? (
            <div className="max-w-xl mx-auto bg-white rounded-lg shadow-md p-6">
              <h3 className="text-2xl font-bold text-blue-700 mb-6 text-center">Nossos Destaques por Nota</h3>
              {safelocalMaisAvaliado.map((local, index) => (
                <div key={local.id} className="flex items-center justify-between border-b border-gray-200 py-3 last:border-b-0">
            ...
            <p className="text-center text-xl text-gray-600 mt-10">Nenhum local no ranking ainda. Avalie e ajude a construir nosso ranking!</p>
          )}
        </div>
      </section>
    </Layout>
...
// --- getServerSideProps ---
// Esta função é executada no lado do servidor a cada requisição para a página.
// Ela busca os dados da API e os passa como 'props' para o componente HomePage.
export const getServerSideProps: GetServerSideProps<HomePageProps> = async () => {
  try {
    // Tenta buscar os dados de todas as APIs em paralelo para otimizar o tempo de carregamento.
    const [locals, avaliacoes, localMelhorAvaliado] = await Promise.all([
      fetchLocal(),
    ...
    // Em caso de erro em qualquer uma das chamadas da API, registra e retorna uma mensagem de erro.
    ...
    return {
      props: {
        ...
        avaliacoes: [],
        localMelhorAvaliado: [],
        error: error.message || 'Erro desconhecido ao carregar os dados.',
      },
    };
  }
};
